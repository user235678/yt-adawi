...
export default function UpdateUserModal({ userId, isOpen, onClose }: UserProfileModalProps) {
  ...
  useEffect(() => {
    if (isOpen && userId) {
      // Utiliser les routes vendeur au lieu des routes admin
      fetcher.load(`/seller/users?intent=getUserProfile&userId=${userId}`);
    }
  }, [isOpen, userId]);
                ...
                <div className="bg-gray-50 p-4 rounded">
                  {userData.photo && userData.photo.length > 0 ? (
                    <div className="grid grid-cols-3 gap-4 mb-4">
                      {userData.photo.map((photo: string, idx: number) => (
                        <img
                          key={idx}
                          src={photo}
                          alt={`Photo ${idx + 1}`}
