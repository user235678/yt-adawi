            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white rounded-lg p-4 shadow-md">
                        ...
                        <p className="text-2xl font-bold">{total_revenue} EUR</p>
                    </div>
                    <div className="bg-white rounded-lg p-4 shadow-md">
                    ...
                    <div className="bg-white rounded-lg p-4 shadow-md">
                        ...
                        <p className="text-2xl font-bold">{commission_collected} EUR</p>
                    </div>
                </div>
                ...
                <ul className="mb-6">
                    {revenue_evolution.map((item, index) => (
                        <li key={index} className="flex justify-between">
                            <span>{item.date}</span>
                            <span>{item.revenue} EUR ({item.orders_count} commandes)</span>
                        </li>
                    ))}
                </ul>
                ...
                <ul className="mb-6">
                    {sales_by_category.map((category, index) => (
                        <li key={index} className="flex justify-between">
                            ...
                            <span>{category.total_sales} EUR ({category.product_count} produits)</span>
                        </li>
                    ))}
                </ul>
                ...
                <ul className="mb-6">
                    {geographic_sales.map((sale, index) => (
                        <li key={index} className="flex justify-between">
                            ...
                            <span>{sale.sales_count} ventes, {sale.revenue} EUR</span>
                        </li>
                    ))}
                </ul>
                ...
                <ul className="mb-6">
                    {recent_orders.map((order, index) => (
                        <li key={index} className="flex justify-between">
                            ...
                            <span>{order.total} EUR - {order.status}</span>
                        </li>
                    ))}
                </ul>
            </div>
